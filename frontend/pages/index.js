...
export default function Home() {
    ...
    const [showAdminQR, setShowAdminQR] = useState(true);
            ...
            <div className="max-w-3xl mx-auto">
                ...
                {roomId && (
                    <div className="mt-8 space-y-6">
                        ...
                        {/* Toggle Admin QR Button */}
                        <div className="flex justify-center mt-4">
                            <button
                                onClick={() => setShowAdminQR(!showAdminQR)}
                                className="px-6 py-2 rounded-lg font-semibold hover:opacity-90 transition-colors text-sm"
                                style={{ backgroundColor: '#8b5cf6', color: '#ffffff' }}
                            >
                                {showAdminQR ? '👑 Hide Admin QR' : '👑 Show Admin QR'}
                            </button>
                        </div>
                    </div>
                )}
